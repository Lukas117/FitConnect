<script>
  ...
  const selectedEventEndDate = $selectedEvent.end_date ? new Date($selectedEvent.end_date) : null;
...
<div class="flex pt-14 pb-24">
    ...
    <div class="absolute left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2 w-0.5 h-[50vh] bg-black"
         style="top: calc(50% + 23px);"></div>
...
<div class="flex flex-col items-center mt-44">
    <button on:click={togglePause} class={`text-black transition-transform duration-300 ease-in-out ${
                          isPaused ? 'transform -rotate-180' : ''}`}>
        <svg class="w-16 h-16 text-primary fill-primary rotate-180" fill="none" stroke="currentColor"
             viewBox="0 0 24 24"
             xmlns="http://www.w3.org/2000/svg">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                  d="{isPaused ? 'M5 3l14 9L5 21V3z' : 'M6 4h4v16H6zM14 4h4v16h-4z'}"></path>
        </svg>
    </button>
</div>
